...
func init() {
	...
	flag.StringVar(&dstS3Bucket, "dst", "s3://exampleBucket", "Destination S3 Bucket")
	...
	flag.IntVar(&stopAfter, "stopafter", 5, "Abort everything after this many")
	flag.Parse()
	...
	if !strings.HasPrefix(dstS3Bucket, "s3://") {
		fmt.Fprintf(os.Stderr, "The destination bucket (%q) must start with \"s3://\"\n", dstS3Bucket)
		os.Exit(1)
	}
